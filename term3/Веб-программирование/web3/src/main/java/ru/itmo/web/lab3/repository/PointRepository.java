...
public class PointRepository {
    ...
    public void save(Point point) {
        System.out.println("Попытка сохранить точку: " + point);
        try (Connection conn = getConnection()) {
            System.out.println("Подключение к БД установлено: " + DatabaseProperties.getUrl());
            try (PreparedStatement pstmt = conn.prepareStatement(
                ...
                try (ResultSet rs = pstmt.getGeneratedKeys()) {
                    if (rs.next()) {
                        ...
                        System.out.println("Сохранено с ID: " + point.getId());
                    }
                }
            }
        } catch (SQLException e) {
            e.printStackTrace(); // важно временно включить!
